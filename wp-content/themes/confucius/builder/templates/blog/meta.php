...
<div class="tm_pb_post_meta"><?php
	...
	if ( 'on' === $this->_var( 'show_author' ) ) {
		echo tm_get_safe_localization(
			sprintf(
				'<span class="tm_pb_post_meta_author">' . __( 'By %s', 'confucius' ) . '</span>',
	...
	if ( 'on' === $this->_var( 'show_date' ) ) {
		echo tm_get_safe_localization(
			sprintf(
				'<span class="tm_pb_post_meta_date">' . __( '%s', 'confucius' ) . '</span>',
	...
	if ( 'on' === $this->_var( 'show_comments' ) ) {
		printf(
			...
			esc_html(
				_nx( '1 Comment', '%s Comments', get_comments_number(), 'number of comments', 'confucius' )
			) . '</span>',
