...
class Confucius_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'confucius' );
		$this->widget_id          = 'confucius_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'confucius' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'confucius' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'confucius' ),
				'label' => esc_html__( 'Subscribe Title', 'confucius' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'confucius' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'confucius' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'confucius' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'confucius' ),
				'label' => esc_html__( 'Subscribe submit label', 'confucius' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'confucius' ),
				'label' => esc_html__( 'Subscribe success', 'confucius' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'confucius' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'confucius' ),
				'label' => esc_html__( 'Follow Title', 'confucius' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'confucius' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'confucius' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'confucius' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'confucius' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'confucius' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'confucius' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'confucius' ),
					'top-center'    => esc_html__( 'Top Center', 'confucius' ),
					'top-right'     => esc_html__( 'Top Right', 'confucius' ),
					'center-left'   => esc_html__( 'Middle Left', 'confucius' ),
					'center'        => esc_html__( 'Middle Center', 'confucius' ),
					'center-right'  => esc_html__( 'Middle Right', 'confucius' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'confucius' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'confucius' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'confucius' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'confucius' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'confucius' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'confucius' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'confucius' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'confucius' ),
					'contain' => esc_html__( 'Contain', 'confucius' ),
					'auto'    => esc_html__( 'Auto', 'confucius' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'confucius' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_confucius_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_confucius_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'confucius' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'confucius' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'confucius' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'confucius' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'confucius' );
		}
	...
	public function get_social_nav() {
		return confucius_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = confucius_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'confucius_subscribe' ) ) {
...
add_action( 'widgets_init', 'confucius_register_subscribe_follow_widgets' );
function confucius_register_subscribe_follow_widgets() {
	register_widget( 'Confucius_Subscribe_Follow_Widget' );
}
