 ...
 * @package Confucius
 */
if ( ! class_exists( 'Confucius_Instagram_Widget' ) ) {
	class Confucius_Instagram_Widget extends Cherry_Abstract_Widget {
		/**
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of photos from Instagram network.', 'confucius' );
			$this->widget_id          = 'confucius_widget_instagram';
			$this->widget_name        = esc_html__( 'Instagram', 'confucius' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'Instagram', 'confucius' ),
					'label' => esc_html__( 'Title', 'confucius' ),
				),
				'tag' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Hashtag (enter without `#` symbol)', 'confucius' ),
				),
				'image_counter' => array(
					'type'       => 'stepper',
					'value'      => '6',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Number of photos', 'confucius' ),
				),
				'display_caption' => array(
					...
					'options' => array(
						'display_caption_check' => esc_html__( 'Caption', 'confucius' ),
					),
				),
				'display_date' => array(
					'type'  => 'checkbox',
					...
					'options' => array(
						'display_date_check' => esc_html__( 'Date', 'confucius' ),
					),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( empty( $instance['tag'] ) ) {
				return print $args['before_widget'] . esc_html__( 'Please, enter #hashtag.', 'confucius' ) . $args['after_widget'];
			}
			ob_start();
			...
			if ( ! $photos ) {
				return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered a invalid hashtag.', 'confucius' ) . $args['after_widget'];
			}
			$template = locate_template( 'inc/widgets/instagram/views/instagram.php', false, false );
			printf( '<div class="%s">',
				join( ' ', apply_filters( 'confucius_instagram_widget_wrapper_class', array( 'instagram__items' ) ) )
			);
		...
		public function get_photos( $data, $img_counter, $config ) {
			 ...
			 * @var string
			 */
			$order = apply_filters( 'confucius_instagram_widget_order', 'media' );
			...
			foreach ( $nodes as $photo ) {
				...
				if ( in_array( 'caption', $config ) ) {
					$_photo['caption'] = wp_html_excerpt(
						$photo['caption'],
						apply_filters( 'confucius_instagram_widget_caption_length', 10 ),
						apply_filters( 'confucius_instagram_widget_caption_more', '&hellip;' )
					);
				}
		...
		public function get_tags_url() {
			return apply_filters( 'confucius_instagram_widget_get_tags_url', $this->service_url . 'explore/tags/%s/' );
		}
		/**
		...
		public function get_post_url() {
			return apply_filters( 'confucius_instagram_widget_get_post_url', $this->service_url . 'p/%s/' );
		}
		/**
		 * Clear cache.
		 *
	...
	add_action( 'widgets_init', 'confucius_register_instagram_widget' );
	function confucius_register_instagram_widget() {
		register_widget( 'Confucius_Instagram_Widget' );
	}
}