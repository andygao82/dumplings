 ...
 * @return array
 */
function confucius_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! confucius_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'confucius_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'confucius_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'confucius' ) . $network['name'],
